...
float last_time = 0;
float movement_duration = 2000;
int step_size = 1;
//float[] targets_x = new float[]{-0.3, 0, -0.3};
//float[] targets_y = new float[]{0.1, 0.25, 0.1, 0, -0.15, -0.25, -0.15, 0};
int index = 0;
PFont f;
...
void keyPressed() {
  if (key == 'q') {
    ...
    looptime = int(1000*random(-0.1, 0.8));
    cp5.getController("looptime").setValue(looptime);
  }
}
